...
class AddSeriesTest extends StatelessWidget {
	AddSeriesTest({super.key, required this.subjectModel, required this.seriesTestMarkModel});

	final SubjectModel subjectModel;
	final List<SeriesTestMarkModel> seriesTestMarkModel;
						...
						children: [
							Container(
								...
								child: CustomTextField(
									...
									validator: (input) => input == '' ? 'Please enter title' : null,
								),
							),
							Expanded(
								child: Container(
									...
									child: ListView.builder(
										itemBuilder: (context, index) {
											return Container(
												...
												child: ListTile(
													onTap: () => seriesTestMarkModel[index].focusNode.requestFocus(),
													...
													trailing: SizedBox(
														...
														height: 35,
														child: TextFormField(
															controller: seriesTestMarkModel[index].markController,
															focusNode: seriesTestMarkModel[index].focusNode,
															onFieldSubmitted: (value) {
																if(index+1 != seriesTestMarkModel.length) {
																	seriesTestMarkModel[index+1].focusNode.requestFocus();
																}
															},
															onChanged: (value) {
																if(value.length > 2) {
																	seriesTestMarkModel[index].markController.text = value.substring(0,2);
																	seriesTestMarkModel[index].markController.selection = TextSelection.fromPosition(
																		TextPosition(offset: seriesTestMarkModel[index].markController.text.length),
																	);
																}
															},
	...
	void saveSeriesTestData(BuildContext context) {
		if(_addSeriestestFormKey.currentState!.validate()) {
			...
			if(isAnyMarkFieldEmpty(seriesTestMarkModel)) {
				customAlertDialog(
					context: context,
					messageText: "Empty marks will be taken as '0'. Click Save to continue",
	...
	Future<void> addSeriesTestData(BuildContext context) async {
		...
		final List<SeriesTestMarkModel> seriesTestMarksObjects = seriesTestMarkModel;
		...
		try {
			...
			await FirebaseFirestore.instance.collection('seriesTests').doc(docId).set(seriesTestModel.toMap());
			showSnackBar(
				context: context,
	...
	bool isAnyMarkFieldEmpty(List<SeriesTestMarkModel> list) {
		for(SeriesTestMarkModel model in list) {
